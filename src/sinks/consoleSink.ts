...
export function createConsoleSink(): LogSink {
  return {
    ...
    write: (evt: LogEvent) => {
      ...
      if (evt.level === "debug") console.debug(head, evt.message, ...extras);
      else if (evt.level === "info") console.info(head, evt.message, ...extras);
      else if (evt.level === "warn") console.warn(head, evt.message, ...extras);
      else console.error(head, evt.message, ...extras);
    },
  };
}
